...
export default function ChatRoomScreen() {
  ...
  const handleSend = async (imageUrl?: string) => {
    ...
    try {
      ...
      // Even if it fails here, it might have been added to the offline queue
      // Firestore usually doesn't throw for offline writes unless it's a permission error
    } finally {
      setSending(false);
    }
  };
    ...
    return (
      <View className={`flex-row mb-4 ${isMe ? 'justify-end' : 'justify-start'}`}>
        ...
        <View 
          ...
          } ${item.pending ? 'opacity-70' : 'opacity-100'}`}
        >
          {item.imageUrl && (
          ...
          <View className="flex-row items-end justify-between">
            {item.text && (
              <Text className={`text-sm font-medium ${isMe ? 'text-white' : 'text-slate-800'} mr-2`}>
                {item.text}
              </Text>
            )}
            {isMe && item.pending && (
              <Clock size={10} color="white" className="mb-0.5" />
            )}
          </View>
        </View>
      </View>
